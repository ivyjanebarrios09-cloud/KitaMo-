...
function PostCard({ post, chairpersonId, roomId, role }: { post: Post; chairpersonId: string; roomId: string; role: 'chairperson' | 'student' }) {
    ...
    const seenByNames = useMemo(() => {
        if (!seenBy || seenByLoading) return [];
        // This part can be enhanced by fetching user names from a 'users' collection
        return seenBy.map(s => s.userId.substring(0, 5) + '...');
    }, [seenBy, seenByLoading]);
    ...
    return (
        <Card className="w-full">
            ...
            <CardFooter className="flex justify-between items-center">
                 {role === 'student' && (
                    <Button onClick={handleMarkAsSeen} disabled={hasSeen}>
                 ...
                 {role === 'chairperson' && (
                    <div className="flex items-center text-sm text-muted-foreground gap-2">
                        {seenByLoading || membersLoading ? <Skeleton className="h-5 w-20" /> : (
                            <>
                                <Users className="h-4 w-4" />
                                <TooltipProvider>
                                    <Tooltip>
                                        <TooltipTrigger asChild>
                                             <span className="font-medium cursor-pointer">{seenBy?.length || 0} of {members?.length || 0} students have seen this.</span>
                                        </TooltipTrigger>
                                        <TooltipContent>
                                            {seenByNames.length > 0 ? seenByNames.join(', ') : 'No one has seen this yet.'}
                                        </TooltipContent>
                                    </Tooltip>
                                </TooltipProvider>
                            </>
